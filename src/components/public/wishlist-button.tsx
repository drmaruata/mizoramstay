...
export function WishlistButton({ propertyId, initialSaved = false }: { propertyId: string; initialSaved?: boolean }) {
  ...
  return (
    <button
      ...
      disabled={busy}
      onClick={toggle}
      className="grid size-9 place-items-center rounded-full bg-white/90 transition hover:bg-white disabled:opacity-60"
    >
      <Heart className={`size-4 ${saved ? 'fill-[#d4942f] text-[#d4942f]' : 'text-[#17332e]'}`} />
    </button>
  )
}
