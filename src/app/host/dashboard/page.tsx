...
export default async function HostDashboardPage() {
  ...
  const [stats, hostProfile] = await Promise.all([
    hostProfileId
      ? service.getDashboardStats(hostProfileId)
      : Promise.resolve({
          todayCheckIns: 0,
          todayCheckOuts: 0,
          upcomingBookings: 0,
    ...
    hostProfileId
      ? db.from('host_profiles').select('display_name').eq('id', hostProfileId).maybeSingle()
  ...
  const profileName = hostProfile.data?.display_name?.trim()
  const emailName = user.email?.split('@')[0]?.replace(/[._-]/g, ' ')
  const fallbackName = emailName?.replace(/\b\w/g, (letter) => letter.toUpperCase())
  const hostName = profileName || fallbackName || 'Host'
      ...
      <div className="space-y-6 md:space-y-7">
        <header className="flex flex-col gap-5 rounded-[28px] border border-[#d9e1dc] bg-[#f8f6f0] p-5 shadow-[0_10px_30px_rgba(21,70,55,.04)] sm:flex-row sm:items-end sm:justify-between md:p-7">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-[-.045em] text-[#17332e] sm:text-[40px]">Welcome, {hostName}</h1>
          ...
          <Link href="/host/calendar" className="shrink-0">
            <Button className="h-11 rounded-xl bg-[#154637] px-5 text-white shadow-[0_8px_18px_rgba(21,70,55,.14)] hover:bg-[#103b2f]">
              <CalendarDays className="size-4 text-white" /> View calendar <ArrowRight className="size-4 text-white" />
            </Button>
          </Link>
        </header>
        ...
        {!hostProfileId ? (
          <Card className="overflow-hidden rounded-[28px] border-[#d9e1dc] shadow-sm">
            <CardContent className="grid gap-6 p-6 md:grid-cols-[1fr_auto] md:items-center md:p-8">
              ...
              <Link href="/host/properties"><Button className="rounded-xl bg-[#154637] text-white hover:bg-[#103b2f]">Manage properties <ArrowRight className="size-4 text-white" /></Button></Link>
            </CardContent>
          </Card>
        ) : (
          <>
                      ...
                      {stats.recentBookings.map((b) => (
                        <div key={b.id} className="group flex flex-col gap-4 rounded-2xl border border-[#e2e7e3] p-4 transition hover:border-[#bdcec5] hover:bg-[#fbfcfa] sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <div className="flex items-center justify-between gap-3 sm:justify-end">
                            ...
                            <Link href="/host/bookings"><Button variant="outline" size="sm" className="h-9 rounded-xl border-[#d7dfda] bg-white px-3 text-[#17332e]">View <ArrowUpRight className="size-3.5 text-[#17332e]" /></Button></Link>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <h2 className="mt-2 text-xl font-black tracking-[-.03em] text-white">{stats.pendingRequests > 0 ? 'Review pending requests' : stats.upcomingBookings > 0 ? 'Keep upcoming stays ready' : 'Make your listing bookable'}</h2>
                      </div>
                    ...
                    <Link href={stats.pendingRequests > 0 ? '/host/bookings' : stats.upcomingBookings > 0 ? '/host/calendar' : '/host/properties'} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-bold text-[#154637] transition hover:bg-[#f3efe6]">
                      {stats.pendingRequests > 0 ? 'Open bookings' : stats.upcomingBookings > 0 ? 'Prepare calendar' : 'Manage listing'} <ArrowRight className="size-3.5 text-[#154637]" />
                    </Link>
                  </CardContent>
                </Card>
