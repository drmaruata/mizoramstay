...
export async function POST(request: Request) {
  try {
    ...
    const {data,error}=await db.rpc('create_booking_transaction_v2',{p_property_id:parsed.data.propertyId,p_room_id:parsed.data.roomId,p_check_in:parsed.data.checkIn.toISOString().slice(0,10),p_check_out:parsed.data.checkOut.toISOString().slice(0,10),p_guests:parsed.data.guests,p_quantity:1,p_guest_name:guestName,p_guest_phone:parsed.data.guest.phone,p_guest_email:parsed.data.guest.email??null,p_notes:null,p_idempotency_key:idempotencyKey,p_hold_minutes:15})
