  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: { code: 'UNAUTHENTICATED', message: 'Please sign in before booking.' } },
        { status: 401 }
      )
    }
    ...
    const { data, error } = await db.rpc('create_booking_transaction', {
      ...
      p_check_in: parsed.data.checkIn.toISOString().slice(0, 10),
      p_check_out: parsed.data.checkOut.toISOString().slice(0, 10),
      p_guests: parsed.data.guests,
      p_quantity: 1,
      p_guest_name: guestName,
      p_guest_phone: parsed.data.guest.phone,
      p_guest_email: parsed.data.guest.email ?? null,
      p_notes: null,
    ...
    if (error) {
      const status = ['P0003', 'P0004'].includes(error.code ?? '') ? 409 : error.code === '42501' ? 401 : 400
      return NextResponse.json(
        { error: { code: error.code ?? 'BOOKING_FAILED', message: error.message } },
        { status }
      )
    }
    ...
    if (!booking) {
      return NextResponse.json(
        { error: { code: 'BOOKING_FAILED', message: 'Booking hold could not be created.' } },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json(
      ...
      { status: 201, headers: { 'Idempotency-Key': idempotencyKey } }
    )
  } catch (error) {
    console.error('[create-booking]', error)
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Could not create booking.' } },
      { status: 500 }
    )
  }
}
