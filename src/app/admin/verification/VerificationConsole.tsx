...
export function VerificationConsole({ properties }: { properties: VerificationProperty[] }) {
  ...
  function toggleChange(key: string) {
    setSelectedChanges((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])
  }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
      <section className="rounded-[28px] border border-[#d9e2dd] bg-white shadow-sm">
        <div className="border-b border-[#edf1ee] p-4"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#7e8e87]">Verification queue</p><h2 className="mt-1 text-xl font-black">Submitted properties</h2><div className="mt-4"><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search host, property, district" /></div></div>
